                    ...
                    if len(current_sequence) >= args.min_frames:
                        ...
                        if top:
                            ...
                            if args.practice_target:
                                ...
                                if not target_match:
                                    full_results = predict(sequence=current_sequence, templates=templates, target_len=args.resample, top_k=len(templates))
                                ...
                                if target_match:
                                    ...
                                    is_top = (best["word"] == target_word)
                                    
                                    # Calculate margin
                                    if is_top:
                                        next_conf = top[1]["confidence"] if len(top) > 1 else 0.0
                                        margin = conf - next_conf
                                    else:
                                        margin = conf - best["confidence"] # Negative margin

                                    if is_top and conf >= args.practice_min_confidence and margin >= args.practice_min_margin:
                                        evaluation = "CORRECT!"
                                        color = (0, 255, 0) # Green
                                    elif is_top or (conf >= args.practice_min_confidence * 0.8):
                                        evaluation = "ALMOST CORRECT"
                                    ...
                                    last_prediction = f"[{evaluation}] {target_word.upper()}: {conf:.1f}%"
                                    print(f"\n[PRACTICE EVALUATION] {evaluation} | Target: {target_word.upper()} | Conf: {conf:.1f}% | Margin: {margin:.1f}%")
                                else:
