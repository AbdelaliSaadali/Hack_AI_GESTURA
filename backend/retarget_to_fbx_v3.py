...
def finger_bone_names(bone_map: Dict[str, str]) -> List[str]:
    ...
    for side in ("left", "right"):
        for logical_suffix, _, _ in FINGER_SEGMENTS:
            key = f"{side}_{logical_suffix}"
            if key in bone_map:
...
def animate_fingers_for_frame(
    ...
    for side in ("left", "right"):
        for logical_suffix, a_name, b_name in FINGER_SEGMENTS:
            bone_key = f"{side}_{logical_suffix}"
            bone_name = bone_map.get(bone_key)
            if not bone_name:
                continue
            a = get_hand_point(frame_data, side, a_name)
            b = get_hand_point(frame_data, side, b_name)
            target_dir = calibrated_direction(a, b, mirror_x, depth_scale)
